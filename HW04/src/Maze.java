...
public class Maze {
	...
	/**
	 * This method will recursively call itself to build the arbitrary MazeRoute that 
	 * is made up from the first returned MazeCell from the connected cells of the 
	 * last MazeCell added to the MazeRoute.
	...
	public MazeRoute routeRandom(MazeCell initialCell) throws UninitializedObjectException {
		...
		if (mazeCells.contains(initialCell)) {
			buildRandomMazeRoute(initialCell);
	...
	/**
	 * This method will recursively call itself to build the MazeRoute's List randomly. 
	...
	//Complexity = 3
	private void buildRandomMazeRoute(MazeCell cell) {
		try {
			if (addThisCell(cell)) {
				route.add(cell);
				mousesPath.add(cell);
				MazeCell nextCell = getNextRandomCell(cell);
				buildRandomMazeRoute(nextCell); //Recursive call to continue building the route.
			}
			else if (!mazeCells.contains(cell)) {
				//If the cell is not a part of this Maze, end the recursion and remove all of the MazeCells in the route.
				route.removeAll(route);
	...
	//Complexity = 0
	private MazeCell getNextRandomCell(MazeCell cell) {
		MazeCell nextCell = new MazeCell();
